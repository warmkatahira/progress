...
<x-app-layout>
    <div class="py-5 mx-5">
        ...
        <div class="grid grid-cols-12 gap-4">
            ...
            <a href="{{ route('customer.delete', ['customer_code' => $customer->customer_code]) }}" id="customer_delete" class="col-start-10 xl:col-start-12 col-span-3 xl:col-span-1 bg-red-600 text-white rounded-lg text-center text-base xl:text-2xl py-3">削除</a>
        </div>
        <!-- アラート -->
        <x-alert/>
        <!-- 変更フォーム -->
        <form method="POST" action="{{ route('customer.modify') }}" class="m-0 grid grid-cols-12">
            ...
            <div class="col-span-12 grid grid-cols-12 mt-5 gap-y-2">
                ...
                <select name="is_detail_available" class="col-span-8 xl:col-span-2">
                    <option value="0" @if($customer->is_detail_available == 0) selected @endif>無効</option>
                    <option value="1" @if($customer->is_detail_available == 1) selected @endif>有効</option>
                </select>
